  ...
  C: CollectionMarker,
{
  ...
  /// Directory where the stores and collection metadata will be saved.
  /// An explicit path takes precedence over any path saved in metadata.
  #[must_use]
    ...
    M: Manager<R>,
  {
    ...
    let colocated_meta = self.path.is_some();
    let meta = Meta::read(app, plugin_name, self.path.as_deref())?;
    let path = self
      .path
      .take()
      .or(meta.inner.path)
