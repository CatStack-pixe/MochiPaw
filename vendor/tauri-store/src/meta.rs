...
impl Meta {
  pub(crate) fn read<R, M>(app: &M, name: &str, explicit_path: Option<&Path>) -> Result<MetaGuard>
  where
    R: Runtime,
    M: Manager<R>,
  {
    read(&meta_file_path(app, name, explicit_path)?)
  }
    ...
    C: CollectionMarker,
  {
    ...
    let explicit_path = collection
      .colocated_meta
      .then_some(collection_path.as_path());
    let path = meta_file_path(&collection.app, &collection.name, explicit_path)?;
    let mut meta = read(&path)?;
    meta.inner.path = Some(collection_path);
...
fn meta_file_path<R, M>(app: &M, name: &str, explicit_path: Option<&Path>) -> Result<PathBuf>
where
  R: Runtime,
  M: Manager<R>,
{
  // Do not resolve or read the platform config directory for an explicit collection.
