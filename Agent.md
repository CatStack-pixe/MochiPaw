...
| 猫咪设置 | `src/pages/preference/components/cat/index.vue` | 猫咪窗口、透明度、缩放、置顶、猫咪锁定等配置 | 主要写入 `useCatStore()` |
...
| 修改窗口置顶/猫咪锁定/任务栏 | `src/plugins/window.ts`、`src-tauri/src/plugins/window/src/commands/*`、`src/pages/main/index.vue` | 平台实现不同，macOS 主窗口使用 NSPanel |
