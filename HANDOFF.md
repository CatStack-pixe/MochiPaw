...
- README Features wording (2026-08-26): replaced the implementation-oriented feature list with user-facing descriptions covering customizable Live2D pets, keyboard/mouse/gamepad-driven behaviors, multi-model management, Pomodoro and typing statistics, desktop controls, Windows compatibility, and Windows/macOS/Linux support.

- External fork Git audit (2026-08-26): fetched the requested fork into a temporary mirror and inspected its full master history. The fork's `master` and `v1.2.1` tag point to `f37b9b9`; after the shared `b20b953` base it contains only `dd92fda` (`feat(gamepad): support non-XInput controllers via WGI backend`) and `f37b9b9` (`chore(release): prepare v1.2.1`). The feature commit switches gilrs from XInput-only to its WGI-capable default backend, retries backend enumeration, adds device/event diagnostics, and adds a General settings gamepad diagnostics panel. It also adds `.pnpm-store` ignoring, pnpm build-script allow-listing, and locale keys; most locale diff volume is line-ending normalization. The release commit only bumps versions to `1.2.1`. The fork does not include the later local `origin/master` Windows Raw Input work. No source changes were made.

- README presentation (2026-08-26): centered the existing `public/logo.png` above the project name, placed the download, latest stable release, and current `package.json` version badges in the first row, and moved the MIT plus PolyForm Noncommercial badge to a second centered row. Badge links and Markdown formatting were checked; no source or workflow changes were made.

- Prerelease published (2026-08-25): PR #101 was labeled `release` and `windows`, passed Frontend, Rust Linux, Rust Windows, and Portable Unicode smoke CI, received a findings-first audit with no blocking or actionable findings, and was squash-merged as `970ee01`. The annotated tag `v1.2.1-1` points to that merge commit. Release workflow run `32871318692` completed all five platform builds, signing, Windows portable packaging, updater metadata verification, and publication successfully. GitHub Release [MochiPaw v1.2.1-1](https://github.com/CatStack-pixe/MochiPaw/releases/tag/v1.2.1-1) is public and marked Pre-release with 24 uploaded assets, `latest-v2.json` containing version `1.2.1-1` and 15 platform mappings, no legacy `latest.json`, and the final `## What's New` description. Real interactive Windows acceptance testing remains recommended for desktop, hidden/locked cursor, windowed/borderless/fullscreen, and elevated game scenarios.
...
- Release preparation (2026-08-24): normalized the package/Cargo versions to `1.2.0`, added Pomodoro to persistence recovery, and kept `latest-v2.json` as the only current updater metadata asset. The 1.1.x updater key is retired, so 1.1.6 clients must cross the signing-key boundary with one manual 1.2.0 installation; aliasing new metadata as `latest.json` would cause verification failure. PR #97 passed Frontend, Rust Linux, Rust Windows, and Portable Unicode smoke CI, received a findings-first audit with no actionable findings, and was squash-merged as `e4ec2e5`. The final tag and Release are now recorded above.
...
- Git comparison (2026-08-24): local `master`, `origin/master`, and `origin/HEAD` all point to `b11c1f4` (`fix(input): restore keyboard events after model migration`); local and origin are `0/0` ahead/behind, and `git ls-remote` confirms no newer origin commit. Local `master` is 135 commits ahead of `upstream/master` and 0 behind. The worktree has 20 uncommitted paths with `268` insertions and `119` deletions, covering release-version metadata (`1.2.1-1`), structured diagnostics for device/gamepad/update/Pomodoro flows, frontend listener and updater error handling, and deletion of two VS Code settings files. No source changes were made during this comparison.

- Corrected the next preview release version to `1.2.1-1` in `package.json`, `src-tauri/Cargo.toml`, and the `mochi-paw` package entry in `Cargo.lock`. `pnpm exec tsx scripts/checkReleaseVersion.ts v1.2.1-1` passed. This is a local version correction only; no commit, tag, push, PR, review, merge, or GitHub Release was created.

- Log audit (2026-08-21): reviewed `MochiPaw.log` (20 lines, 16.9 KiB, last write 02:52). No `WARN`, `ERROR`, `CRITICAL`, exception, traceback, crash, timeout, or failed-operation entries were present; the only levels were 5 `DEBUG` and 15 `INFO`. The Tao event-loop messages (`NewEvents emitted without explicit RedrawEventsCleared` and `RedrawEventsCleared emitted without explicit MainEventsCleared`) are debug-level scheduling diagnostics, not a failure by themselves. The updater reports stable `1.2.0` as available while the running package is prerelease `1.2.0-2`; this is expected SemVer behavior because every hyphen suffix is a prerelease and therefore ranks below the matching stable version. The project's intended sequence must instead advance the next base version before adding a preview suffix, for example `1.2.0 < 1.2.1-1 < 1.2.1-2 < 1.2.1`; using `1.2.0-1` or `1.2.0-2` after publishing `1.2.0` is incompatible with SemVer updater ordering. Same-second focus records contain contradictory combinations such as `focused=false` with `hasFocus=true`, which likely reflects asynchronous event/document-state sampling and makes the diagnostic field unreliable. The file modification time (02:52) is about 33 minutes later than the last event timestamp (02:19:34), so the file may have been touched or rewritten without new entries. The log ends without a trailing CRLF, which is harmless. No source changes were made for this audit.

- Built the unsigned Windows MSI for `v1.2.0-2`: [MochiPaw_1.2.0-2_x64_en-US.msi](E:\文件\Python%20Script\Mochi\MochiPaw\target\release\bundle\msi\MochiPaw_1.2.0-2_x64_en-US.msi). Build command: `pnpm exec tauri build --bundles msi --no-sign`. Artifact size: 7.10 MiB; SHA-256: `AB5B7B631DA85EC61469E44565C5E575E8519BA4C36C028818D92651FB2982DE`. Version consistency verified with `pnpm exec tsx scripts/checkReleaseVersion.ts v1.2.0-2`. Updater signing was intentionally skipped because this is an unsigned local build.

- Expanded operational diagnostics across the native device, gamepad, update-capability, and updater flows. Listener lifecycle transitions, backend selection, startup timeouts, queue drops, Wayland protocol failures, gamepad session errors, package detection failures, update check/download/install stages, and event emission errors now reach the Tauri log without recording raw input values. Frontend Tauri subscriptions, native device/gamepad startup, device status refresh, model-load failures, administrator relaunch, model-import cleanup, runtime telemetry, and Pomodoro coordination now include structured context instead of silent or duplicate console output. Verification: `pnpm test` (160/160), `pnpm exec tsc --noEmit`, focused ESLint for the updater (passes with no warnings), full ESLint (passes with the repository's existing one-warning ceiling), `cargo test --manifest-path src-tauri/Cargo.toml --all-targets --locked` (31/31), `cargo check --manifest-path src-tauri/Cargo.toml --all-targets --locked`, targeted Rust formatting, and `git diff --check` passed. Full `cargo fmt --all -- --check` still reports unrelated pre-existing formatting drift outside the modified files.

- Fixed keyboard input appearing unresponsive after model-selection migration. `src/utils/modelStorePersistence.ts` now drops the legacy persisted `currentModel` object once schema v2 has a stable `currentModelId` and migration is complete; retaining that stale object could overwrite the runtime-selected model during Pinia hydration, causing input resources to be scanned from a different/nonexistent path and leaving `supportKeys` empty. Added regression coverage for completed and pending migration states in `src/utils/modelStorePersistence.test.ts`. Verification: `pnpm test` (160/160), focused persistence tests (12/12), TypeScript, ESLint, and `git diff --check` passed. A foreground `cmd` running `pnpm tauri dev` is active (Vite `http://localhost:1420`, Tauri PID `26396`) for manual keyboard testing.

- Built the local unsigned Windows MSI for `v1.2.0-1` from merged `master` commit `61b7001`: `target/release/bundle/msi/MochiPaw_1.2.0-1_x64_en-US.msi` (7.10 MiB, SHA-256 `C81A7C07AD041839881525A9BDD82CA247F9D3D3BF5FF3EF8AED780F0AF35E13`). Command: `pnpm exec tauri build --bundles msi --no-sign`. The installer was produced successfully; updater signing was intentionally skipped because this is a local build.
