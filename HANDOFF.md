...
- Prerelease preparation (2026-08-25): preparing `v1.2.1-1` for the Windows Raw Input compatibility work in commit `1de5958`. Version sources are synchronized to `1.2.1-1`. The planned GitHub Pre-release notes cover background Raw Input relative mouse movement, visible-cursor absolute desktop behavior, hidden/locked-cursor game behavior, backend diagnostics and cleanup, frontend virtual-cursor synchronization, and regression coverage. The release will be created from the repository release workflow after a labeled PR, findings-first audit, and squash merge.
