...
import type { CubismRuntimeDiagnostics } from './core/runtimeDiagnostics'
import { Live2DLoadError } from './core/runtimeDiagnostics'

let cubismRuntimeUsers = 0

function acquireCubismRuntime(): void {
  if (cubismRuntimeUsers === 0) {
  ...
  cubismRuntimeUsers += 1
}

function releaseCubismRuntime(): void {
  cubismRuntimeUsers = Math.max(0, cubismRuntimeUsers - 1)

  if (cubismRuntimeUsers === 0 && CubismFramework.isInitialized()) {
...
export class Live2DSprite extends Sprite {
  ...
  /** Rejector captured from the _readyPromise constructor for readable load errors. */
  private _readyReject: ((reason?: unknown) => void) | null = null
  private _cubismRuntimeAcquired = false
  ...
  getRuntimeDiagnostics(): CubismRuntimeDiagnostics {
    return {
      ...this._model?.getRuntimeDiagnostics(),
      ...this._modelRenderer?.getRuntimeDiagnostics(),
    }
  }
  ...
  private async initModel(): Promise<void> {
    ...
    if (!this._ctx.initialize(canvas, this.getCanvasViewport(), rendererGl)) {
      throw new Live2DLoadError(
        'renderer-init',
        'Live2D could not initialize a WebGL2 rendering context.',
      )
    }
