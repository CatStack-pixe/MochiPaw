...
import { getCubismMocVersion, getCubismRuntimeDiagnostics, Live2DLoadError } from '../core/runtimeDiagnostics'
...
export class ModelLoader {
    ...
    try {
      if (typeof WebGL2RenderingContext === 'undefined' || !(gl instanceof WebGL2RenderingContext)) {
        throw new Live2DLoadError(
          'webgl2-unavailable',
          'Live2D Cubism 5.3 requires a WebGL2 rendering context.',
      ...
      throw new Live2DLoadError(
        'model-resource',
        `Failed to load Live2D model: ${error instanceof Error ? error.message : String(error)}`,
        {
          ...getCubismRuntimeDiagnostics(),
          ...model.getRuntimeDiagnostics(),
        },
        error,
      )
    }
  ...
  private async loadMoc(model: Live2DModel, ctx: ResolvedSetting): Promise<void> {
    ...
    if (!fileName) {
      throw new Live2DLoadError(
        'model-resource',
        'The model3.json file does not declare a MOC3 model.',
        getCubismRuntimeDiagnostics(),
      )
    }
    ...
    if (!mocVersion) {
      throw new Live2DLoadError(
        'moc-invalid',
        `The referenced MOC3 file is invalid or empty: ${fileName}`,
        diagnostics,
      )
    }

    if (diagnostics.latestMocVersion !== undefined && mocVersion > diagnostics.latestMocVersion) {
      throw new Live2DLoadError(
        'moc-incompatible',
        `The model uses MOC3 version ${mocVersion}, but this Cubism Core supports up to version ${diagnostics.latestMocVersion}.`,
        diagnostics,
      )
    }
