...
import { getCubismRuntimeDiagnostics } from '../core/runtimeDiagnostics'
...
export class Live2DModel extends CubismUserModel {
  ...
  private _mocVersion: number | undefined
  private _lipsync = true
  ...
  loadMocModel(buf: ArrayBuffer): void {
    this._mocVersion = this.getMocVersionFromBuffer(buf)
    this.loadModel(buf, this._mocConsistency)

    if (!this.getModel()) {
      throw new Error('Cubism Core rejected the MOC3 model.')
    }
  }
  ...
  draw(matrix: CubismMatrix44, frameBuffer: WebGLFramebuffer, viewport: Viewport): void {
    ...
    this.getRenderer().drawModel(Config.ShaderPath)
  }
  ...
  getRuntimeDiagnostics(): CubismRuntimeDiagnostics {
    ...
    return {
      ...
      blendModeEnabled: model?.isBlendModeEnabled(),
    }
  }
